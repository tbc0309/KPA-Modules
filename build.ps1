...
if ($Module -in @('all','touch')) {
    $TouchDir = Join-Path $ProjectRoot 'touch'
    $Version = Get-ModuleVersion $TouchDir
    $BinDir = Join-Path $TouchDir 'bin'
    New-Item -ItemType Directory -Force -Path $BinDir | Out-Null
    $env:GOOS = 'android'
    $env:GOARCH = 'arm64'
    $env:CGO_ENABLED = '0'
    & go build -trimpath -ldflags='-s -w' -o (Join-Path $BinDir 'kpa_touch_guard') (Join-Path $TouchDir 'src/main.go')
    if ($LASTEXITCODE -ne 0) { throw 'Go build failed.' }
    $Output = Join-Path $Dist "KPA_Touch_Guard_v$Version.zip"
    Compress-Module $TouchDir $Output
    Write-Host "Built $Output"
}
